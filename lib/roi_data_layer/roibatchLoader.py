...
class roibatchLoader(data.Dataset):
  def __init__(self, roidb, ratio_list, ratio_index, query, batch_size, num_classes, training=True, normalize=None, seen=True):
    ...
    # rajath
    self.coco_sketchy_map = pickle.load(open("../data/coco_sketchy_map.pkl", "rb"))
    self.sketchy_classes = np.array([int(class_idx) for class_idx in self.coco_sketchy_map if len(self.coco_sketchy_map[class_idx]["sketchy"]) > 0]).astype(np.uint8)
  ...
  def filter(self, seen):
    if seen==1:
      ...
      if len(self.list)==1:
        ...
        self.list = [self._classes[cat] for cat in cfg.train_categories]
      ...
      if len(self.list)==1:
        ...
        self.list = [self._classes[cat] for cat in cfg.test_categories]
      ...
      if len(self.list)==2:
        ...
        self.list = [self._classes[cat] for cat in cfg.train_categories + cfg.test_categories]

    self.list_ind = [self._classes_inv[x] for x in self.list]
    # self.list_ind = self.sketchy_classes
